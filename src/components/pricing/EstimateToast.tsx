...
export default function EstimateToast({ selectedItems, data, discountTiers, onClose, onClear }: EstimateToastProps) {
  const estimate = useMemo(() => {
    ...
    const selectedData = data.filter(item => selectedItems.includes(item.area) && item.setEligible);
    ...
    for (const tier of discountTiers) {
      if (selectedData.length >= tier.places) {
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="border-t border-gray-200 pt-3 space-y-2">
            ...
            {estimate.discountRate > 0 && (
              <>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-green-600">{estimate.discountLabel}</span>
                  <span className="text-green-600">-{formatPrice(estimate.savings)}</span>
                </div>
          ...
          <div className="text-xs text-gray-500 bg-gray-50 p-2 rounded">
            ※ この見積りは参考価格です。実際の料金は無料カウンセリングにてご相談ください。
          </div>
        </div>
      </div>
    </div>
